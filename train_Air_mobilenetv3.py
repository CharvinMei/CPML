...
from model_MobileNetV3_small import CPML
...
with open(os.path.join(exp_dir, 'train_log_mobilenetv3_CPML.csv'), 'w+') as file:
...
for epoch in range(nb_epoch):
    ...
    with torch.no_grad():
        ...
        ##### Logging
        with open(os.path.join(exp_dir, 'train_log_mobilenetv3_CPML.csv'), 'a+') as file:
            file.write(
                '{}, {:.4f}, {:.4f}, {:.3f}%, {:.3f}%\n'.format(
        ...
        if eval_acc > max_eval_acc:
            ...
            torch.save(
                net.state_dict(),
                os.path.join(exp_dir, 'max_acc_mobilenetv3_CPML.pth'),
